      ...
      <Page size="A4" style={styles.page}>
          ...
          {items.map((it, idx) => (
            <View style={styles.tRow} key={idx}>
              <View style={[styles.tCell, styles.colDesc]}>
                ...
                {it.serialNo ? <Text style={{ fontSize: 7, color: "#555", marginTop: 2 }}>S/N: {it.serialNo}</Text> : null}
              </View>
        ...
        <View style={styles.totalsBox}>
          <View style={styles.totalsRow}>
            <Text style={styles.totalsLabel}>Total Value of Supply (excl. tax)</Text>
        ...
        {invoice.additional_info ? (
          <View style={styles.infoNoteBox}>
            <Text style={{ fontWeight: 700, marginBottom: 2 }}>Additional Information:</Text>
            {invoice.additional_info
              .split("\n")
              .map((s) => s.trim())
              .filter(Boolean)
