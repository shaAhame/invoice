...
// Atomically gets the next invoice number for a branch. The counter is
// continuous per branch — it never resets month to month — so the trailing
// number just keeps climbing forever and never repeats, even though the
// YYMMM part of the invoice number changes with whatever month it's issued in.
// It's seeded with the configured starting number only the very first time
// this branch is ever used.
// `forDate` lets you generate a number for a backdated invoice — the YYMMM
// prefix reflects that date instead of today, while the trailing number still
// just continues the normal running sequence for the branch.
async function getNextInvoiceNumber(branchCode, forDate) {
  const pool = getPool();
  const ym = yearMonthCode(forDate || new Date());
  const startingNumber = STARTING_NUMBERS[branchCode] || 1;

  const { rows } = await pool.query(
    `INSERT INTO invoice_counters (branch_code, last_number)
     VALUES ($1, $2)
     ON CONFLICT (branch_code)
     DO UPDATE SET last_number = invoice_counters.last_number + 1
     RETURNING last_number;`,
    [branchCode, startingNumber]
  );
...
// branch (i.e. nothing was generated after it, in any month), the counter is
...
async function deleteInvoice(id) {
  ...
  try {
    ...
    const [, branchCode, numStr] = invoice.invoice_no.split("-");
    ...
    const { rows: counterRows } = await client.query(
      `SELECT last_number FROM invoice_counters WHERE branch_code = $1 FOR UPDATE;`,
      [branchCode]
    );
